 ...
 */
const clientConfig = {
  input: path.resolve(__dirname, './client/index.ts'),
  plugins: [
    typescript({
      target: 'es2018',
      include: ['./client/**/*.ts'],
  ...
  const nodeConfig = {
    ...
    plugins: [
      alias({
        ...
        entries: {
          '@vue/compiler-dom': require.resolve('@vue/compiler-dom/dist/compiler-dom.cjs.js'),
        },
      }),
...
function licensePlugin() {
  return license({
    thirdParty(dependencies) {
      ...
      const coreLicense = fs.readFileSync(path.resolve(__dirname, '../../LICENSE'));
